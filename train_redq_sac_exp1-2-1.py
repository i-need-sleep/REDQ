...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='./data/1-2-1/')
    ...
    hidden_list = [(256,256), (512,512), (128,128,128,128)]
    seed_list = [0, 1]
        ...
        for env_name in env_names:
            for seed in seed_list:
                if setting_to_run == current_setting: # only happens when setting value matches
                    ...
                    exp_name_full = '%s_poly%s_%s' % (exp_name_prefix, str(hidden_size), env_name)
                    ...
                    redq_sac(env_name, seed=seed, epochs=epochs,
                         logger_kwargs=logger_kwargs, debug=args.debug,
                             utd_ratio=utd_ratio, num_Q=num_Q, auto_alpha=auto_alpha, alpha=alpha, hidden_sizes=hidden_size)
                current_setting += 1
