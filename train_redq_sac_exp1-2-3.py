        ...
        for env_name in env_names:
            for seed in seed_list:
                if setting_to_run == current_setting: # only happens when setting value matches
                    ...
                    exp_name_full = '%s_poly%s_%s' % (exp_name_prefix, str(batch_size), env_name)
                    ...
                    redq_sac(env_name, seed=seed, epochs=epochs,
                         logger_kwargs=logger_kwargs, debug=args.debug,
                             utd_ratio=utd_ratio, num_Q=num_Q, auto_alpha=auto_alpha, alpha=alpha, batch_size=batch_size)
                current_setting += 1
